...
NetRecvUntilChar proc uses ebx s, stream, maxlen, char, timeout
        LOCAL   lpBuf: BYTE
...
szHTTPSendFmt	db	"POST %s HTTP/1.0",13,10
				...
				db	"User-Agent: Mozilla/4.0 (compatible; MSIE 5.0; Windows 98)",13,10,13,10,0
...
NetWorks proc uses ebx edi s, lpOutStream, lpszRedir
	...
	xor	ebx, ebx
@l:
	invoke  NetRecvUntilChar, s, stream, 64000, 0ah, 30
	test    eax, eax
	jz      @ht_ret_close
...
MyDownloadWithRedir proc uses edi ebx szLink, lpOutStream, lpszRedir
	...
	invoke	wsprintf, pFmt, offset szHTTPHdrFmt, pURL, pHost
...
MyUpload proc uses edi ebx szLink, lpData, dwLen, lpOutStream
	...
	invoke	wsprintf, pFmt, offset szHTTPSendFmt, pURL, pHost, dwLen
