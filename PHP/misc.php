...
// decode 4 byte array into int32 value
...
// correct UTF encoding is extracted from the data header, see detect_utf_encoding() function
	function UnkTextToAnsi($string)
	{
		...
		switch ($encoding)
		{
			case "UTF-8": $string = substr($string, 3);
				break;
			case "UTF-32BE": $string = substr($string, 4);
				break;
			case "UTF-32LE": $string = substr($string, 4);
				break;
			case "UTF-16BE": $string = substr($string, 2);
				break;
			case "UTF-16LE": $string = substr($string, 2);
				break;
			default:
...
// 64-bit blocks only
	...
	{
		require_once("includes/geoip/geoip2.inc");
	}
	else
	{
